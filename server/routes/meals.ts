...
const FALLBACK: GenerateMealsResponse = {
	meals: [
		{ name: 'Lemon Garlic Salmon', description: 'Salmon baked with fresh lemon and garlic.', tags: ['salmon','lemon','garlic'], source: 'fallback' },
		{ name: 'Pesto Pasta', description: 'Classic basil pesto tossed with pasta and parmesan.', tags: ['basil','parmesan','pasta'], source: 'fallback' },
		{ name: 'Chicken Caesar Wrap', description: 'Grilled chicken, lettuce and dressing in a tortilla.', tags: ['chicken','lettuce','tortilla'], source: 'fallback' },
		{ name: 'Veggie Stir Fry', description: 'Mixed vegetables quickly stir-fried in savory sauce.', tags: ['broccoli','carrot','soy sauce'], source: 'fallback' },
	],
};
...
{ "meals": [ { "name": string, "description": string, "tags": string[] } ] }
Do not include any explanatory text outside JSON.`;
		...
		const meals = parsed.meals
			...
			.map((m: any) => ({
				...
				description: (m.description ? String(m.description) : '').trim().slice(0, 200),
